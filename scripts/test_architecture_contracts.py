#!/usr/bin/env python3
"""Regression proof untuk multi-pack, multi-institution, dan academic period generic."""
...
from ramu_repo import ROOT, RepoError, load_json, scope_identity
...
def check_suite_identity(manifest: dict) -> None:
    ctx = {"id": manifest["id"], "manifest": manifest}
    for suite in manifest["eval_suites"]:
        scope = suite["scope"]
        expected = scope_identity(ctx, scope)
        actual = suite.get("scope_ref")
        if scope == "core":
            if actual not in {None, ""}:
                raise AssertionError(f"{manifest['id']}: core tidak boleh memiliki scope_ref")
        elif actual != expected:
            raise AssertionError(
                f"{manifest['id']}: suite {suite['id']} scope_ref {actual!r} != identity {expected!r}"
            )
...
def main() -> int:
    ...
    if not any(item.startswith("trimester-") for item in periods):
        raise AssertionError("Fixture harus membuktikan period non-semester: trimester.")
    if not any(item.startswith("term-") for item in periods):
        raise AssertionError("Fixture harus membuktikan period non-semester: term.")
    ...
    try:
        check_suite_identity(wrong_scope)
